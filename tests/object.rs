...
fn test_object() {
  ...
  assert_eq!(*activitystream_object.get_content(), Some(ActivityStreamMultilangString::String(content)));

  let context = String::from("ActivityTest-Context");
  activitystream_object.set_context(context.clone());
  assert_eq!(*activitystream_object.get_context(), Some(ActivityStreamMultilangString::String(context)));
